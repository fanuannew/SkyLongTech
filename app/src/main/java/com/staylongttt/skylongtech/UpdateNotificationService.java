...
public class UpdateNotificationService extends Service {
    ...
    public void onStart(Intent intent, int startId) {
        ...
        //通知图标
        updateNotification.icon = R.mipmap.ic_launcher;//日後需要替換
        //通知信息描述
        updateNotification.tickerText = "正在下载 " + appName;
        updateNotification.when = System.currentTimeMillis();
        updateIntent = new Intent(this, MainActivity.class); //日後需要檢查
        ...
        protected void onPostExecute(Integer result) {
            ...
            if (result == DOWNLOAD_COMPLETE) {
                ...
                updatePendingIntent = PendingIntent.getActivity(
                        UpdateNotificationService.this, 0, installIntent, 0);
                updateNotification.contentIntent = updatePendingIntent;
                updateNotification.contentView.setTextViewText(
                        R.id.download_notice_speed_tv,
                        "下载完成"); //getString(R.string.update_notice_finish)
                ...
                updateNotificationManager.notify(0, updateNotification);
                //启动安装程序
                UpdateNotificationService.this.startActivity(installIntent);
                stopSelf();
                ...
                updateNotification.contentView.setTextViewText(
                        R.id.download_notice_speed_tv,
                        "下载失败"); //getString(R.string.update_notice_nomemory)
                updateNotification.flags |= Notification.FLAG_AUTO_CANCEL;
                updateNotification.defaults = Notification.DEFAULT_SOUND;
                updateNotificationManager.notify(0, updateNotification);
                stopSelf();
                ...
                updateNotification.contentView.setTextViewText(
                        R.id.download_notice_speed_tv,
                        "下载失败"); //getString(R.string.update_notice_error)
                updateNotification.flags |= Notification.FLAG_AUTO_CANCEL;
                updateNotification.defaults = Notification.DEFAULT_SOUND;
                updateNotificationManager.notify(0, updateNotification);
                stopSelf();
